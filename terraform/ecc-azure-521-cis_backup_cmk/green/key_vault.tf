...
resource "azurerm_key_vault_access_policy" "backupvault" {
  ...
  tenant_id    = azurerm_data_protection_backup_vault.this.identity.0.tenant_id
  object_id    = azurerm_data_protection_backup_vault.this.identity.0.principal_id
...
resource "azurerm_data_protection_backup_vault_customer_managed_key" "this" {
  ...
  depends_on = [ azurerm_key_vault_access_policy.backupvault ]
}
